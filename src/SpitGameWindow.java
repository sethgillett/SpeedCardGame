...
public class SpitGameWindow extends PApplet {
	...
	public void draw() {
		...
		if (sc.playerWon()) state = WIN;
		else if (sc.aiWon()) state = LOSE;
		...
		if (state == MENU) {
			...
			// AI controller attempts to move every draw cycle, fewer errors than multithreaded approach
			aiController.play();
		}
	}
